...
export default function Home() {
  ...
  useEffect(() => {
    axios.get("https://testdemo-backend-strapi-production.up.railway.app/api/features?populate=image")
      .then(res => {
          ...
          (res.data.data || [])
            .map(item => ({
              ...
              image: Array.isArray(item.image) && item.image[0]?.url
                ? "https://testdemo-backend-strapi-production.up.railway.app" + item.image[0].url
                : ""
            }))
        );
      })
